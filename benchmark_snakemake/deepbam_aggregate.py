...
def read_file_to_dict(file_path, t):
    ...
    with open(file_path, 'r') as file:
        ...
        for line in file:
            cols = line.strip().split('\t')  # split by tab
            key = (cols[3], int(cols[4]))  # contig_id, read_posiion
...
def main():
    ...
    parser.add_argument('--filter_ratio', type=float, help='Proportion of rows to filter out based on uncertainty_interval | e.g., 0.1 for 10per.')
    ...
    summary_df.to_csv(summary_output, sep='\t', index=False, header=False)
